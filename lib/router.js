...
TransferFeedsController = RouteController.extend({
  template: 'feedsList',
  increment: 5,
  postsLimit: function () {
    return parseInt(this.params.postsLimit) || this.increment;
  ...
  subscriptions: function () {
    this.postsSub = Meteor.subscribe('transfernews', this.findOptions());
  },
  posts: function () {
    return transfer_news(this.findOptions());
  },
  nextPath: function () {
    return Router.routes.transferFeeds.path({ postsLimit: this.postsLimit() + this.increment });
  },
  fastRender: true,
  data: function () {
    var self = this;
...
AnalysisFeedsController = RouteController.extend({
  template: 'feedsList',
  increment: 5,
  postsLimit: function () {
    return parseInt(this.params.postsLimit) || this.increment;
  ...
  subscriptions: function () {
    this.postsSub = Meteor.subscribe('analysisnews', this.findOptions());
  },
  posts: function () {
    return analysis_news(this.findOptions());
  },
  nextPath: function () {
    return Router.routes.analysisFeeds.path({ postsLimit: this.postsLimit() + this.increment });
  },
  fastRender: true,
  data: function () {
    var self = this;
...
Router.route('/transfer/:postsLimit?', {
  name: 'transferFeeds',
  controller: TransferFeedsController
});

Router.route('/analysis/:postsLimit?', {
  name: 'analysisFeeds',
  controller: AnalysisFeedsController
});
